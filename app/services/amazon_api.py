...
class AmazonAPI:
    ...
    def _generate_headers(self) -> Dict:
        ...
        string_to_sign = self._create_string_to_sign(canonical_request, date_stamp, amz_date)
    ...
    def _create_string_to_sign(self, canonical_request: str, date_stamp: str, amz_date: str) -> str:
        return "\n".join([
            "AWS4-HMAC-SHA256",
            amz_date,
    ...
    def _create_auth_header(self, signature: str, date_stamp: str, amz_date: str) -> str:
        return (
        ...
        for item in response.get("SearchResult", {}).get("Items", []):
            try:
                results.append({
                    ...
                    "price": float(item["Offers"]["Listings"][0]["Price"]["DisplayAmount"].replace("$", "")),
                    "category": item.get("BrowseNodeInfo", {}).get("BrowseNodes", [{}])[0].get("Name", "General"),
